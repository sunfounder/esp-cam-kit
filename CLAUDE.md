...
The project tutorial `.rst` files previously used embedded Arduino Cloud iframes to display code. These have been/will be replaced with inline `.. code-block:: arduino` directives containing the actual code.
...
- Removed "18650" from two text descriptions:
  - "3.7V 18650 battery" → "3.7V battery" (line ~24)
  - "3.7V 18650 lithium batterry" → "3.7V lithium batterry" (line ~93)
- Commented out the entire **"Battery Power and Charging"** section using RST comments (`..`), including:
...
- **`ar_iot_html_cam_led.rst`**: Commented out the "Insert a battery into the expansion board..." step and its `plugin_battery.png` image
...
Removed excessive `:align: center` and `:width: XX%` attributes from image directives throughout the Blynk configuration tutorial. Most images now use only `:width:` (absolute pixel values like 600 or 700) without centering. This affects ~25+ image directives across sections 2.1 through 3 (Blynk signup, template creation, datastreams, web dashboard, events, device creation, and code sections).

### Supporting Config Changes (same round)

- **`conf.py`**: Project name changed from "SunFounder ESP32 Starter Kit" to "SunFounder ESP32 Camera Kit"; removed `autosectionlabel` extension; added `sphinx_rtd_theme` to extensions and theme options
- **`requirements.txt`**: Updated `sphinx==7.3.7`, `sphinx_rtd_theme==3.0.2`; removed `readthedocs-sphinx-search`
